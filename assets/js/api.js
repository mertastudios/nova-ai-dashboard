...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new ApiError(
      'Der Server ist gerade nicht erreichbar. Pruefe deine Internetverbindung und versuche es erneut.',
      0,
      'network_error'
    );
  }
  ...
  try {
    ...
    // Leerer Body ist z.B. bei 204-Antworten normal.
  }

  if (!res.ok) {
    if (res.status === 401) clearToken();
    const message = data?.error || `Unbekannter Fehler (Status ${res.status}).`;
